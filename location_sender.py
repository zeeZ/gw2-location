...
def main():
    ...
    parser.add_argument('-p','--port',default=8888,type=int,dest='port',help='Destination port')
    parser.add_argument('-s','--server',required=True,dest='host',help='Destination server')
    parser.add_argument('-q','--quiet',action='store_true',dest='quiet',help="Don't print out all that crap")
    ...
    try:
        ...
        ws = websocket.WebSocketApp("ws://%s:%d/publish" % (args.host, args.port))
        ws.on_open = on_open
        ws.run_forever()
